...
if(isset($_GET['p']) && !empty($_GET['p'])){
	...
	if ($conn->query($del) === TRUE) {
		...
		$conn->query($delp);
		header("location: dead-stock-products.php?d=del");		
	} else {
		header("location: dead-stock-products.php?d=n");		
	}
}
if(isset($_GET['deadtock']) && !empty($_GET['deadtock'])){
	$deadtock=strip_tags(trim($_GET['deadtock']));
	$del = "UPDATE `product_detail` SET `dead_stock` = '0' WHERE `product_detail`.`productdetail_id` = '$deadtock'";
	if ($conn->query($del) === TRUE) {		
		header("location: dead-stock-products.php?d=y");		
	} else {
		header("location: dead-stock-products.php?d=n");		
	}
}
...
<head>
		...
		<!-- Title -->
		<title><?php echo $lang['DeadStockProduct'];?></title>
	...
	<body class="fixed-sidebar fixed-header skin-3 content-appear">
			...
			<div class="site-content">
				...
				<div class="content-area py-1">
					<div class="container-fluid">
						<h4><?php echo $lang['DeadStockProductsList'];?></h4>
						<ol class="breadcrumb no-bg mb-1">
							<li class="breadcrumb-item"><a href="index.php"><?php echo $lang['Dashboard'];?></a></li>							
							<li class="breadcrumb-item active"><?php echo $lang['DeadStockProductsList'];?></li>
						</ol>
						<div class="box box-block bg-white">
							<h5 class="mb-1"><?php echo $lang['ExportingDeadStockProductsData'];?></h5>
							<table class="table table-striped table-bordered dataTable" id="table-2">
								...
								<tbody>
                                <?php								
								$sql = "SELECT prInfo.*, prDetail.* FROM product as prInfo, product_detail as prDetail where prInfo.product_id=prDetail.p_id and prDetail.dead_stock='1'";
								...
								if ($result->num_rows > 0) 
								{
									...
									<tr>
										...
										<td><?php if(isset($setting_currency)){ echo $setting_currency;}?><?php echo $row['product_sell_price'];?></td>
                                        ...
                                        <a class="btn btn-secondary btn-sm" href="dead-stock-products.php?deadtock=<?php echo $row['productdetail_id'];?>" title="Dead Stock"><i class="ti-info-alt mr-0-5"></i><?php echo $lang['NotDeadStock'];?></a>
                                        ...
                                        <a class="btn btn-danger btn-sm" href="dead-stock-products.php?p=<?php echo $row['product_id'];?>" title="Delete"><i class="ti-trash mr-0-5"></i><?php echo $lang['Delete'];?></a>
                                        <?php } ?>
                                        </td>
									</tr>
                                  <?php $i++; }
	  ...
	  if(isset($_GET['d']) && $_GET['d']=='n'){ ?>
        ...
        <script type="text/javascript">
			...
			toastr.error('Products move to Stock Successfully!');
