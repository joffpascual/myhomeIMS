...
if(isset($_GET['i']) && !empty($_GET['i'])){
	...
	$sql = "SELECT a.auth_username,a.auth_id,o.*,inv.*, ipd.* FROM sup_invoice_payment_detail as ipd,
	 suppliers_orders as o,sup_invoices as inv, auth as a where a.auth_id=ipd.auth_id 
	and a.auth_id=o.auth_id and ipd.auth_id=o.auth_id and ipd.exp_invoice_id=inv.exp_invoice_number 
	and o.exp_order_id=ipd.exp_order_id and o.exp_order_id='$getorder_id'";
				...
				<form class="form-vertical" id="editexpinvoice" name="editexpinvoice" enctype="multipart/form-data" method="post" accept-charset="utf-8">
                    <div class="panel-body">
                        <div class="row">                        
							<div class="col-md-6">
                                ...
                                <div class="form-group row">
									<label for="supplier_id" class="col-sm-3 form-control-label"><?php echo $lang['Suppliers'];?> <i class="text-danger">*</i></label>
									<div class="col-sm-6">										
                                        ...
                                        <?php while($rowc = $resultc->fetch_assoc()) { ?>
											<option <?php if($supplier_id==$rowc['supplier_id']){echo "selected";}?> value="<?php echo $rowc['supplier_id'];?>"><?php echo $rowc['supplier_name'];?></option>
										<?php } ?>	
										</select>
                                ...
                                <div class="form-group row">
									...
									<div class="col-sm-6">
										<select id="warehouse_id" name="warehouse_id"  class="form-control" data-plugin="select2">
											...
											<option <?php if($warehouse_id==$roww['war_id']){echo "selected";}?> value="<?php echo $roww['war_id'];?>"><?php echo $roww['war_name'];?></option>
										<?php } ?>
										</select>
									</div>
								</div> 
                        ...
                        <div class="table-responsive" style="margin-top: 10px">
							...
							$sqlquery = "SELECT * from sup_invoice_line_items where exp_order_id=$getorder_id and exp_invoice_id=$exp_invoice_id";
								...
								{	
                                    ...
                                    <tr>
                                        <td style="width: 320px">
                                            <input name="product_name[]" class="form-control productSelection" value="<?php echo $productrow['exp_product_name'];?>" placeholder="Item Name" required id="product_name" autocomplete="off" tabindex="1" type="text">
                                        </td>                                        
...
<script type="text/javascript">
			jQuery(document).ready(function(){			
			  ...
			  $('#editexpinvoice').on('submit', function(e) {				
				NProgress.start();
				e.preventDefault();   
				$.ajax({
			
						
						url  : 'modal/edit_supplierinvoiceModel.php',
						type: "POST",
				   ...
				   processData:false,
						...
						{					
							...
							else if(data.status==='successfully')
							{
									...
									NProgress.done();
									setTimeout('$("#editexpinvoice").fadeOut(500, function(){ window.location = "suppliers-invoices.php" }); ',1000);
