...
if(isset($_GET['l']) && !empty($_GET['l'])){
	...
	$del = "DELETE FROM loan_needer WHERE loaner_id='$l'";
	if ($conn->query($del) === TRUE) {
		header("location: loaners.php?d=del");		
	} else {
		header("location: loaners.php?d=n");		
	}
}
?>
<!DOCTYPE html>
...
<head>
		...
		<!-- Title -->
		<title><?php echo $lang['Loaner'];?></title>
			...
			<div class="site-content">
				...
				<div class="content-area py-1">
					<div class="container-fluid">
						<h4><?php echo $lang['LoanersList'];?></h4>
						<ol class="breadcrumb no-bg mb-1">
							<li class="breadcrumb-item"><a href="index.php"><?php echo $lang['Dashboard'];?></a></li>							
							<li class="breadcrumb-item active"><?php echo $lang['LoanersList'];?></li>
						</ol>
						<div class="box box-block bg-white">
							<h5 class="mb-1"><?php echo $lang['ExportingLoanersData'];?></h5>
							<table class="table table-striped table-bordered dataTable" id="table-2">
								...
								if ($result->num_rows > 0) 
								{
									...
									<tr>
										...
										<td><?php echo $row['loaner_address'];?></td>
										<td>
                                        <?php if(!empty($_SESSION['user_type_id']) && $_SESSION['user_type_id']=='1') { ?> 
                                        <a class="btn btn-secondary btn-sm" href="edit-loaner.php?l=<?php echo $row['loaner_id'];?>" title="Edit"><i class="ti-pencil mr-0-5"></i><?php echo $lang['Edit'];?></a> 
                                        <a class="btn btn-danger btn-sm" href="loaners.php?l=<?php echo $row['loaner_id'];?>" title="Delete"><i class="ti-trash mr-0-5"></i><?php echo $lang['Delete'];?></a>
                                        <?php } ?>
                                        </td>
									</tr>
                                  <?php $i++; }
