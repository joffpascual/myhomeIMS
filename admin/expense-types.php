...
if(isset($_GET['e']) && !empty($_GET['e'])){
	$e=strip_tags(trim($_GET['e']));
	$del = "DELETE FROM expense_types WHERE expense_type_id='$e'";
	if ($conn->query($del) === TRUE) {
		header("location: expense-types.php?d=del");		
	} else {
		header("location: expense-types.php?d=n");		
	}
}
?>
<!DOCTYPE html>
...
<head>
		...
		<!-- Title -->
		<title><?php echo $lang['ExpenseType'];?></title>
			...
			<div class="site-content">
				...
				<div class="content-area py-1">
					<div class="container-fluid">
						<h4><?php echo $lang['ExpenseTypesList'];?></h4>
						<ol class="breadcrumb no-bg mb-1">
							<li class="breadcrumb-item"><a href="index.php"><?php echo $lang['Dashboard'];?></a></li>							
							<li class="breadcrumb-item active"><?php echo $lang['ExpenseTypesList'];?></li>
						</ol>
						<div class="box box-block bg-white">
							<h5 class="mb-1"><?php echo $lang['ExportingExpenseTypes'];?></h5>
							<table class="table table-striped table-bordered dataTable" id="table-2">
								<thead>
									<tr>
										<th>No</th>
										<th><?php echo $lang['ExpenseType'];?></th>										
								...
								{	
									...
									<tr>
										<td><?php echo $i;?></td>
										<td><?php echo $row['expense_type_name'];?></td>										
										<td>
                                        <?php if(!empty($_SESSION['user_type_id']) && $_SESSION['user_type_id']=='1') { ?> 
                                        <a class="btn btn-secondary btn-sm" href="edit-expense-type.php?e=<?php echo $row['expense_type_id'];?>" title="Edit"><i class="ti-pencil mr-0-5"></i><?php echo $lang['Edit'];?></a> 
                                        <a class="btn btn-danger btn-sm" href="expense-types.php?e=<?php echo $row['expense_type_id'];?>" title="Delete"><i class="ti-trash mr-0-5"></i><?php echo $lang['Delete'];?></a>
                                        <?php } ?>
                                        </td>
									</tr>
                                  <?php $i++; }
