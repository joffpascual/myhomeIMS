...
if(isset($_GET['p']) && !empty(trim($_GET['p']))){
	...
	if ($conn->query($del) === TRUE) {
		...
		$conn->query($delp);
		header("location: products.php?d=del");		
	} else {
		header("location: products.php?d=n");		
	}
}
if(isset($_GET['deadtock']) && !empty(trim($_GET['deadtock']))){
	$deadtock=strip_tags(trim($_GET['deadtock']));
	$del = "UPDATE `product_detail` SET `dead_stock` = '1' WHERE `product_detail`.`productdetail_id` = '$deadtock'";
	if ($conn->query($del) === TRUE) {		
			...
			<div class="site-content">
				...
				<div class="content-area py-1">
					<div class="container-fluid">
						<h4><?php echo $lang['ProductsList'];?></h4>
						<ol class="breadcrumb no-bg mb-1">
							<li class="breadcrumb-item"><a href="index.php"><?php echo $lang['Dashboard'];?></a></li>							
							<li class="breadcrumb-item active"><?php echo $lang['ProductsList'];?></li>
						</ol>
						<div class="box box-block bg-white">
							<h5 class="mb-1"><?php echo $lang['ExportingProductsData'];?></h5>
							<table class="table table-striped table-bordered dataTable" id="table-2">
								<thead>
									<tr>
										...
										<th style="width:270.25px;"><?php echo $lang['Option'];?></th>
									</tr>
								</thead>
								<tbody>
                                <?php								
								$sql = "SELECT prInfo.*, prDetail.* FROM product as prInfo, product_detail as prDetail where prInfo.product_id=prDetail.p_id and prDetail.dead_stock='0'  ORDER BY prDetail.productdetail_id DESC ";
								...
								if ($result->num_rows > 0) 
								{
									...
									<tr>
										...
										<td><a class="btn btn-success btn-sm" href="#" data-toggle="modal" data-target=".large-modal" data-whatever="<?php echo $row['product_id'];?>" title="View"><i class="ti-eye mr-0-5"></i><?php echo $lang['View'];?></a> 
                                           <a class="btn btn-secondary btn-sm" href="products.php?deadtock=<?php echo $row['productdetail_id'];?>" title="Dead Stock"><i class="ti-info-alt mr-0-5"></i><?php echo $lang['DeadStock'];?></a>
                                        ...
                                        <a class="btn btn-danger btn-sm" href="products.php?p=<?php echo $row['product_id'];?>" title="Delete"><i class="ti-trash mr-0-5"></i><?php echo $lang['Delete'];?></a>
                                       <?php } ?>
                                        </td>
									</tr>
                                  <?php $i++; }
