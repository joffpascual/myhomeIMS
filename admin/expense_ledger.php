...
<head>
		...
		<!-- Title -->
		<title><?php echo $lang['ExpenseLedger'];?></title>
			...
			<div class="site-content">
				...
				<div class="content-area py-1">
					<div class="container-fluid">
						<h4><?php echo $lang['ExpensesLedger'];?></h4>
						<ol class="breadcrumb no-bg mb-1">
							<li class="breadcrumb-item"><a href="index.php"><?php echo $lang['Dashboard'];?></a></li>							
							<li class="breadcrumb-item active"><?php echo $lang['ExpensesLedger'];?></li>
						</ol>
						<div class="box box-block bg-white">
							<h5 class="mb-1"><?php echo $lang['ExpenseLedgerData'];?></h5>
							...
							<table class="table table-bordered dataTable" id="s_i_l">
							  ...
							  {
								$sql = "SELECT a.auth_id, a.auth_username,a.full_name,a.user_type_id, inv.* FROM exp_invoice_payment_detail as inv, auth as a where inv.exp_payment_detail_date >='".$_POST['start']."' and inv.exp_payment_detail_date <='".$_POST['end']."' and a.auth_id=inv.auth_id order by inv.exp_payment_detail_date ASC";
							  }else{
							   $newEndingDate = date("Y-m-d", strtotime(date("Y-m-d", strtotime(date("Y-m-d"))) . " - 1 year"));
								$sql = "SELECT a.auth_id, a.auth_username,a.full_name,a.user_type_id, inv.* FROM exp_invoice_payment_detail as inv, auth as a where inv.exp_payment_detail_date <='".date("Y-m-d")."' and inv.exp_payment_detail_date >='$newEndingDate' and a.auth_id=inv.auth_id order by inv.exp_payment_detail_date ASC";
							  }
								...
								if ($result->num_rows > 0) 
								{
                                  ...
                                  <?php $i++;
								  		$gtp=$gtp+$row['exp_grand_total_price'];
										$pa=$pa+$row['exp_paid_amount'];
